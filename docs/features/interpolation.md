...
**Input:** multiplier: 3 items:
- for: [1, 2, 3, 4] num: ${item} tripled: ${item * multiplier} combined: ${item + multiplier}
...
**Input:** config: database: host: localhost port: 5432 connection_string: jdbc:postgresql://localhost:5432/mydb

**Output:** config: database: host: localhost port: 5432 connection_string: jdbc:postgresql://localhost:5432/mydb
...
- for:
  ...
  - name: "scheduler" active: true if: item.active service: ${item.name}
