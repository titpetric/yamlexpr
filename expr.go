...
func New(opts ...ConfigOption) *Expr {
	...
	e.RegisterHandler(e.config.IncludeDirective(), handlers.Include(e, e.config.IncludeDirective()))
	e.RegisterHandler(e.config.ForDirective(), handlers.For(e, e.config.ForDirective()))
	e.RegisterHandler(e.config.MatrixDirective(), handlers.Matrix(e, e.config.MatrixDirective()))
	e.RegisterHandler(e.config.IfDirective(), handlers.If(e.config.IfDirective()))
...
func (e *Expr) processMapWithContext(ctx *model.Context, m map[string]any) ([]any, error) {
	...
	for _, directive := range e.config.HandlerOrder {
		...
		if value, ok := m[directive]; ok {
			...
			results, consumed, err := handler(ctx, m, value)
			if err != nil {
				return nil, err
			}
