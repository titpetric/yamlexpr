...
func TestEvaluateConditionWithPath_Boolean(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, err := handlers.EvaluateConditionWithPath(tt.value, stack.New(nil), "")
...
func TestEvaluateConditionWithPath_InvalidType(t *testing.T) {
	_, err := handlers.EvaluateConditionWithPath([]string{"foo"}, stack.New(nil), "")
	require.Error(t, err)
	require.Contains(t, err.Error(), "unsupported condition type")
}

func TestEvaluateConditionWithPath_InvalidString(t *testing.T) {
	_, err := handlers.EvaluateConditionWithPath("maybe", stack.New(nil), "test.if")
	require.Error(t, err)
